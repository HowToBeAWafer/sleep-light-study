...
The public website contains only the Supabase publishable key. Final-session reads remain protected by Row Level Security (RLS) and a private administrator allow-list. Protocol v5 uses the participant's study-name/password proof to save and resume one profile-linked overnight draft for 48 hours after its most recent save, including after a browser or device change. Historical Protocol v3 bearer-token drafts and Protocol v4 profile drafts remain supported without being converted. Participant credentials are separate from Supabase Auth. The participant website has no participant-email field and does not solicit reminder addresses; participants should not enter an email as their study name or in free-text feedback.
...
> **Protocol v5 production status — verified and published 2026-08-09:** production successfully ran [`supabase/migrations/20260809_protocol_v5.sql`](./supabase/migrations/20260809_protocol_v5.sql). A separate read-only audit confirmed all eight retained/new session constraints are validated; every required v4/v5 validator, participant RPC, administrator RPC, snapshot table and trigger is installed; participant/admin grants, private-table access and RLS policies match the documented boundary; and every invalid-payload, cross-schema condition, profile-link, missing-snapshot and cancelled-reminder count is zero. The historical schema 3 row still has count `1` and fingerprint `b9cf9c7fbb0656882991ce141f221ebf`. Schema 4 currently has count `9` and post-migration baseline fingerprint `3125716abdce83155555cdc8c0699dee`; no exact pre-v5 schema 4 fingerprint was saved, so this is recorded as a new baseline rather than a fabricated before/after proof. A credential-excluding research-data backup and matching SHA-256 manifest were saved before frontend release. Build `2026-08-09-five-session-commitment-v3` was then published to GitHub Pages and its bilingual `SL-V5`, five-session commitment and next-morning questionnaire copy were checked at the public URL. The follow-up build `2026-08-09-v5-progress-refresh-v4` refreshes authenticated progress immediately before every new assignment so a page left open on one device cannot override a newer completion from another device; it requires no database migration. Public recruitment still waits for the non-identifying end-to-end pilot described below. This publication applies to GitHub Pages only; the legacy OpenAI Sites address was not updated to Protocol v5.

## Required 2026-08-09 Protocol v5 migration — run before the v5 frontend

This migration requires the verified 2026-07-31 v4 migration. It is additive: it does not update or reinterpret any v2/v3/v4 payload, and historical v3 `control` remains the retired normal-sleep condition. The new condition ID is `black-control`.

1. Take the appropriate project backup. Run the schema count/fingerprint query printed at the end of `20260809_protocol_v5.sql`, and save the complete result outside SQL Editor.
2. Open **Supabase → SQL Editor → New query**. Delete any older pasted copy, then paste the **entire current** contents of [`supabase/migrations/20260809_protocol_v5.sql`](./supabase/migrations/20260809_protocol_v5.sql). Confirm the first executable line is `begin;`, the last executable line is `commit;`, and the validator contains five explicit `sequence_position`/`candidate_condition` pairs instead of `candidate_condition <> case ...`.
3. Clear any partial text selection, select the entire file, and run it once. A normal result is `Success. No rows returned`. If SQL Editor instead says that the transaction is already aborted, run `rollback;` by itself, open another new query, paste the complete current file again, and retry once.
4. Run the commented verification queries at the bottom of the migration (without the comment prefixes). Confirm the v5 validator, the four participant/progress RPCs, the administrator-only incomplete-session RPC, the private snapshot table and trigger, the documented execute grants, and the session constraints are present.
5. Rerun the count/fingerprint query. Every pre-existing v2/v3/v4 count and fingerprint must exactly match the saved pre-migration result.
6. Before recruitment, use non-identifying pilot data to verify: a profile without eligible history begins at 0/5; completed historical v4 dim red alone carries into effective v5 position 1; completed historical v4 dim red followed by dim blue carries the contiguous prefix into effective positions 1 and 2; v4 bright blue/red never carry; the third assigned session after a two-position carryover is a five-minute black screen with a visible gray cross; all five conditions record four sparse trials, watched time, pauses and interruptions; refresh/cross-browser recovery works; `terminated` does not advance progress; an evening exposure plus immediate post-exposure questionnaire appears in the administrator dashboard even when the morning questionnaire is never submitted; a later final submission replaces that incomplete display with the normal final record; final save, administrator details, CSV/JSON and feedback all work.
7. After the database audit passes, the matching build may be published for controlled pilot testing. Do not begin public recruitment until step 6 passes. The 2026-08-09 GitHub Pages build has been published and smoke-tested; the end-to-end pilot remains pending. Do not disclose the complete condition order in participant-facing pages.

The v5 migration introduces versioned RPCs rather than changing the v4 RPC responses:
...
- `admin_list_incomplete_study_sessions_v5` (authenticated administrator only)

The participant recovery draft still expires 48 hours after its latest save. Separately, whenever a validated schema-v4 or schema-v5 active record has `exposureStatus: "completed"`, a completed `postExposureSurvey`, and no `morningSurvey`, the database appends a distinct private incomplete-night snapshot. Those snapshots do not expire with the participant-facing draft, do not count as completed conditions, and cannot be read directly by anonymous or ordinary authenticated users. The administrator-only RPC returns the newest snapshot for each unresolved session together with `recordStatus: "active"`, `exposureStatus: "completed"`, and one of `awaiting-sleep-start`, `awaiting-morning-return`, or `awaiting-morning-questionnaire`. If that session is later submitted successfully, the final row is preserved and the incomplete version is automatically excluded from the administrator incomplete list; the private snapshot history itself is retained for auditability.

Five-position progress is the union of completed native v5 positions and a narrowly defined historical prefix. The database may treat a completed v4 position 1 (`dim-red`) as effective v5 position 1, and may also treat completed v4 position 2 (`dim-blue`) as effective v5 position 2 only when position 1 is complete. This is a read-time progress calculation: no v4 row, payload, schema, sequence version, questionnaire answer, or timestamp is updated or copied. Historical v4 positions 3/4 (`bright-blue`/`bright-red`) never carry because they were completed before the newly inserted black-screen position and therefore do not satisfy the v5 order. Terminated or active v4/v5 records never advance or carry progress.
...
Restoring the project does not apply any missing migration. Audit which historical migrations are already present, then run only the required remaining migrations in date order. For the current production upgrade, first confirm the already-deployed `20260731_protocol_v4.sql`, then run and verify `20260809_protocol_v5.sql` before the v5 frontend is deployed.
...
8. Run the complete `20260731_protocol_v4.sql` and verify the v4 objects. This migration is also **not** included in `setup.sql`.
9. Save the schema count/fingerprint, then run the complete `20260809_protocol_v5.sql`; verify its v5 objects, fixed-five contract, v4 prefix carryover helper, and unchanged v2/v3/v4 fingerprints. This migration is also **not** included in `setup.sql`.
10. Complete administrator sign-in, password profile creation, bilingual safety/tutorial screens, fixed assignment, v4-prefix carryover cases, practice, all five exposures including black screen/gray cross, the immediate Karolinska Sleepiness Scale, overnight save, same-browser and cross-browser recovery, next-morning questionnaire, final save, administrator details, and CSV/JSON tests before public recruitment or collection of real participant data. A deployed build may be used only for controlled, non-identifying pilot testing until these checks pass.

The current setup script creates the final-session table, RLS policies, private administrator allow-list, v2/v3-compatible constraints, v3 overnight-draft functions, unique-name profiles, progress links, append-only feedback and history-protection triggers. The administrator Auth user must exist before the script runs because its immutable Auth UUID is inserted into the allow-list. `20260718_participant_profiles.sql` is for upgrading the existing project without recreating it. The password, v4 and v5 migrations must follow `setup.sql` in date order because they have intentionally not been folded into the base script.
...
- The legacy anonymous insert policy accepts only final `completed` or `terminated` schema v2/v3 sessions. It does not accept schema v4/v5.
- V4/v5 final records are submitted through their participant-profile RPCs with the matching profile ID and password-derived proof.
...
- The database accepts historical schema v2/v3/v4 and new schema v5 sessions under separate strict contracts without rewriting any format.
- Old v2/v3/v4 answers, including v3 Control, `postSurvey`, and standalone `reactionTest`, are never transformed, overwritten, or automatically deleted. New sessions and feedback are appended with new IDs and version metadata.
...
- V5 requires four planned attention trials in all five positions, including `black-control`, and enforces dim red → dim blue → black control → bright blue → bright red.
- Current v5 progress counts completed native v5 positions plus only the completed contiguous v4 dim-red/dim-blue prefix. This derived progress does not mutate historical records; v4 bright-blue/bright-red never carry into v5.
...
- **A v5 upload is rejected or a v5 RPC is missing:** verify that the complete `20260809_protocol_v5.sql` ran after the verified v4 migration. Do not deploy the v5 page against a v4-only database.
- **V5 progress assigns the wrong condition:** inspect `private.is_v5_sequence_position_completed`. A completed v4 dim-red position 1 may carry; v4 dim-blue position 2 may carry only with completed v4 position 1; v4 bright-blue/red, all v3 records, and active/terminated records must not carry. Never repair progress by updating historical payloads.
