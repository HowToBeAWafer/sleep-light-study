...
- 2026-08-09：构建 `2026-08-09-v5-progress-refresh-v4` 加入开始前强制刷新服务器进度，确保另一设备完成新实验后，当前设备不会沿用缓存条件；生产库无姓名汇总显示旧暗红＋暗蓝账户下一项为黑屏，错误映射为 0。
- 2026-08-10：构建 `2026-08-10-admin-terminated-duration-v5` 让管理员列表中的提前终止记录直接显示有效暴露时长／计划时长、完成比例和“不含暂停”说明；详情与导出的精确原始时长继续保留。
